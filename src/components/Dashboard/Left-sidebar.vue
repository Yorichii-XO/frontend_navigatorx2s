        ...
        <a class="block px-8 py-6 m-0 text-sm whitespace-nowrap dark:text-white text-slate-700" href="https://demos.creative-tim.com/argon-dashboard-tailwind/pages/dashboard.html" target="_blank">
          <img
             class="inline h-full max-w-full transition-all duration-200 dark:hidden ease-nav-brand max-h-8"
              ...
              alt="Website Logo"
            />
          <span class="ml-1 font-semibold transition-all duration-200 ease-nav-brand"> 2s NavigatorX</span>
        </a>
      </div>

      <hr class="h-px mt-0 bg-transparent bg-gradient-to-r from-transparent via-black/40 to-transparent dark:bg-gradient-to-r dark:from-transparent dark:via-white dark:to-transparent" />

      <div class="items-center block w-auto h-full  h-sidenav grow basis-full">
        <ul class="flex flex-col pl-0 mb-0">
          ...
          <router-link to="/users" v-if="isSuperAdmin">
       <li class="mt-0.5 w-full">
        ...
        <router-link to="/activities">
          <li class="mt-0.5 w-full">
            <a class=" dark:text-white dark:opacity-80 py-2.7 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap px-4 transition-colors">
              ...
              <span class="ml-1 duration-300 opacity-100 pointer-events-none ease">Activities of Members</span>
            </a>
          </li>
        </router-link>
        <router-link to="/activities-user">
          <li class="mt-0.5 w-full">
            <a class=" dark:text-white dark:opacity-80 py-2.7 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap px-4 transition-colors">
              <div class="mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-center fill-current stroke-0 text-center xl:p-2.5">
                <i class="relative top-0 text-sm leading-normal text-blue-900 fas fa-user-cog"></i>
              </div>
              <span class="ml-1 duration-300 opacity-100 pointer-events-none ease">Your Activity</span>
            </a>
          </li>
        </router-link>
...
export default {
  data() {
    return {
      currentUser: null, // This will hold the user data fetched from the API
    };
  },
  computed: {
    // Check if the user has the role 'super-admin' based on role_id
    isSuperAdmin() {
      return this.currentUser && this.currentUser.role && this.currentUser.role.id === 3; // Assuming role_id 3 is super-admin
    },
  },
  mounted() {
  ...
  methods: {
    async getCurrentUser() {
      try {
        ...
        // Make an API call to fetch the authenticated user's data
        ...
        this.currentUser = response.data; // Store the user data in currentUser
      } catch (error) {
        console.error('Error fetching user data:', error);
        // Handle the error, for example, redirect to login if not authenticated
      }
    },
  },
};
